...
export default function PractitionerAppointmentsPage() {
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    const res = await fetch(`/api/practitioner/appointments?${params}`)
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#111827]">Mine timer</h1>
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] p-4 mb-6">
        <div className="flex flex-wrap gap-3">
          <input
            type="text"
            placeholder="Søk pasientnavn..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="flex-1 min-w-[200px] px-3.5 py-2 border border-[#D1D5DB] rounded-lg text-sm focus:outline-none focus:ring-2 focus:border-[#0d9463]"
          />
          <input
            type="date"
            value={filterDate}
            onChange={(e) => setFilterDate(e.target.value)}
            className="px-3.5 py-2 border border-[#D1D5DB] rounded-lg text-sm focus:outline-none focus:ring-2 focus:border-[#0d9463]"
          />
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="px-3.5 py-2 border border-[#D1D5DB] rounded-lg text-sm focus:outline-none bg-white"
          >
                  ...
                  <tr key={appt.id} className="hover:bg-[#FAFAFA] transition-colors">
                    ...
                    <td className="hidden sm:table-cell px-4 py-3.5">
                      {appt.payment_status === 'paid' && appt.amount_nok ? (
                        ...
                        <p className="text-xs text-[#9CA3AF]">{appt.payment_status}</p>
                      )}
                    </td>
                  </tr>
                ))}
